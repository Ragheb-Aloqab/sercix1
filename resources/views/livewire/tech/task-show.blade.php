<div class="space-y-6">
    ...
    @if ($errors->any())
        <div class="rounded-2xl border border-red-200 bg-red-50 text-red-900 dark:bg-red-900/20 dark:border-red-800 dark:text-red-300 px-4 py-3">
            <p class="font-bold mb-1">{{ __('livewire.errors_title') }}</p>
    ...
    <div class="rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/70 dark:border-slate-800 shadow-soft p-5">
        <div class="flex items-start justify-between gap-4">
            <div>
                ...
                <p class="text-sm text-slate-500 mt-1">{{ __('livewire.order_status') }}: {{ $order->status }}</p>
                <p class="text-sm text-slate-500 mt-1">
                    {{ __('livewire.company') }}: {{ $order->company?->company_name ?? '-' }}
    ...
    <div class="rounded-3xl bg-emerald-50 dark:bg-emerald-900/20 border-2 border-emerald-200 dark:border-emerald-800 shadow-soft p-5">
        <h3 class="font-black text-lg mb-4 flex items-center gap-2">
            <span class="text-emerald-700 dark:text-emerald-400">{{ __('livewire.driver_info') }}</span>
        </h3>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
            @if($order->requested_by_name)
                <div>
                    <p class="text-slate-500 dark:text-slate-400">{{ __('livewire.driver_name') }}</p>
                    <p class="font-bold mt-1">{{ $order->requested_by_name }}</p>
                </div>
            @endif
            @if($order->driver_phone)
                <div>
                    <p class="text-slate-500 dark:text-slate-400">{{ __('livewire.driver_phone') }}</p>
                    ...
                    <a href="{{ $wa }}" target="_blank" rel="noopener"
                       class="inline-flex items-center gap-2 mt-2 px-3 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-semibold text-sm">
                        ...
                        {{ __('livewire.whatsapp_driver') }}
                    </a>
                </div>
            @endif
            @if($order->vehicle)
                <div>
                    <p class="text-slate-500 dark:text-slate-400">{{ __('livewire.vehicle_label') }}</p>
            ...
            @if($order->address || $order->city)
                <div class="sm:col-span-2">
                    <p class="text-slate-500 dark:text-slate-400">{{ __('livewire.service_location') }}</p>
        ...
        @endif
        @if($order->notes)
            <p class="text-slate-600 dark:text-slate-400 text-sm mt-3"><span class="font-semibold">{{ __('livewire.notes_label') }}:</span> {{ $order->notes }}</p>
        @endif
    </div>
    @endif
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div class="rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/70 dark:border-slate-800 shadow-soft p-5">
            <h3 class="font-black text-lg mb-3">{{ __('livewire.photos_before') }}</h3>
            <form method="POST" action="{{ route('tech.tasks.attachments.before', $order->id) }}"
                ...
                <button type="submit" class="px-4 py-2 rounded-xl bg-sky-600 hover:bg-sky-700 text-white font-semibold">{{ __('livewire.upload') }}</button>
            </form>
        ...
        <div class="rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/70 dark:border-slate-800 shadow-soft p-5">
            <h3 class="font-black text-lg mb-3">{{ __('livewire.photos_after') }}</h3>
            <form method="POST" action="{{ route('tech.tasks.attachments.after', $order->id) }}"
                  enctype="multipart/form-data" class="flex items-center gap-3">
                @csrf
                <input type="file" name="images[]" multiple accept="image/*"
                       class="block w-full text-sm rounded-xl border border-slate-200 dark:border-slate-800 p-2" />
                <button type="submit" class="px-4 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-semibold">{{ __('livewire.upload') }}</button>
            </form>
            <div class="mt-4 grid grid-cols-2 sm:grid-cols-3 gap-3">
    ...
    <div class="rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/70 dark:border-slate-800 shadow-soft p-5">
        <button type="button" wire:click="confirmComplete"
            ...
            {{ __('livewire.confirm_task_complete') }}
        </button>
    </div>
</div>

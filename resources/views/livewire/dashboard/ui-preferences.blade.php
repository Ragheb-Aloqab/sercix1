<div class="flex items-center gap-1 sm:gap-2">
    ...
    <div class="relative" x-data="{ open: false }" @click.outside="open = false">
        <button type="button" @click="open = ! open"
            ...
            <i
                class="fa-solid fa-globe {{ auth('company')->check() ? 'text-cyan-400' : 'text-cyan-500 dark:text-cyan-400' }}"></i>
            <span
                class="hidden sm:inline">{{ app()->getLocale() === 'ar' ? __('dashboard.lang_ar') : __('dashboard.lang_en') }}</span>
        ...
        <div x-show="open" x-transition:enter="transition ease-out duration-200"
            ...
            class="absolute end-0 mt-2 w-40 rounded-xl shadow-soft py-1 z-50 {{ auth('company')->check()
                ? 'bg-slate-800 border border-slate-600/50'
                : 'bg-white dark:bg-slate-900 border border-slate-200/70 dark:border-slate-800' }}"
            style="display: none;">
            <a href="{{ route('set-locale', ['lang' => 'ar', 'return' => '/' . ltrim(request()->path(), '/')]) }}"
                class="block px-4 py-3 min-h-[44px] flex items-center text-sm {{ auth('company')->check()
                    ? (app()->getLocale() === 'ar'
                        ? 'font-bold text-white hover:bg-slate-700/50'
            ...
            <a href="{{ route('set-locale', ['lang' => 'en', 'return' => '/' . ltrim(request()->path(), '/')]) }}"
        ...
        <button wire:click="toggleTheme"
            class="min-w-[44px] min-h-[44px] p-2 sm:px-3 sm:py-2 rounded-xl flex items-center justify-center transition-colors duration-300
               {{ auth('company')->check() || auth('maintenance_center')->check()
                   ...
                   : ($theme === 'dark'
                       ? 'bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 border border-amber-200
                        dark:border-amber-700/50 hover:bg-amber-200 dark:hover:bg-amber-900/50'

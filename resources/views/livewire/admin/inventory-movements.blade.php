<div class="space-y-6" wire:loading.class="opacity-70 pointer-events-none">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <p class="text-lg font-bold text-slate-500 dark:text-slate-400">متابعة جميع عمليات الإدخال والإخراج والتعديلات</p>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div class="bg-white dark:bg-slate-800 rounded-2xl p-4 shadow">
            <p class="text-sm text-slate-500 dark:text-slate-400">إجمالي الداخل</p>
            <h2 class="text-xl font-bold text-emerald-600">+{{ $stats['in'] }}</h2>
        </div>
        <div class="bg-white dark:bg-slate-800 rounded-2xl p-4 shadow">
            <p class="text-sm text-slate-500 dark:text-slate-400">إجمالي الخارج</p>
            <h2 class="text-xl font-bold text-rose-600">-{{ $stats['out'] }}</h2>
        </div>
        <div class="bg-white dark:bg-slate-800 rounded-2xl p-4 shadow">
            <p class="text-sm text-slate-500 dark:text-slate-400">التعديلات</p>
            <h2 class="text-xl font-bold text-amber-500">{{ $stats['adjustments'] }}</h2>
        </div>
        <div class="bg-white dark:bg-slate-800 rounded-2xl p-4 shadow">
            <p class="text-sm text-slate-500 dark:text-slate-400">آخر حركة</p>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl shadow overflow-x-auto">
        <table class="w-full text-sm text-right">
            ...
            <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
                ...
                @empty
                    <tr>
                        <td colspan="8" class="p-8 text-center text-slate-500 dark:text-slate-400">لا توجد حركات مطابقة للفلاتر.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>

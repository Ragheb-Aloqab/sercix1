...
<head>
    ...
    <title>{{ __('login.verify_title') }} — {{ $brandTitle ?? $siteName ?? 'Servx Motors' }}</title>
    @if($siteLogoUrl ?? null)
...
<div class="min-h-screen flex items-center justify-center px-4 py-10">
    <div class="w-full max-w-sm">
        ...
        <div class="bg-servx-black-card rounded-xl border border-servx-red/30 shadow-servx-card p-6 sm:p-8">
            ...
            <a href="{{ route('login') }}" class="mt-4 block text-center text-sm font-medium text-servx-silver hover:text-servx-red transition-colors">{{ __('login.change_phone') }}</a>
        </div>

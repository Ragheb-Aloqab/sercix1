...
<div class="min-h-screen flex flex-col pb-tabbar">
    ...
    <header class="bg-white/95 dark:bg-slate-800/95 border-b border-slate-200 dark:border-slate-600/50 backdrop-blur sticky top-0 z-40 w-full transition-colors duration-300 @if($wlBranding ?? false) topbar-wl-branded @endif">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between gap-3">
            ...
            <div class="flex items-center gap-2 sm:gap-3 shrink-0">
                {{-- Notifications --}}
                <a href="{{ route('driver.notifications.index') }}" class="relative flex items-center justify-center w-10 h-10 rounded-xl border border-slate-300 dark:border-slate-600/50 hover:bg-slate-100 dark:hover:bg-slate-700/50 text-slate-600 dark:text-slate-300 transition-colors" aria-label="{{ __('common.notifications') }}">
                ...
                <div class="flex items-center gap-0.5 rounded-xl border border-slate-300 dark:border-slate-600/50 p-0.5 bg-slate-200/80 dark:bg-slate-800/60">
                    ...
                    <a href="{{ route('set-locale', ['lang' => 'en']) }}" class="px-2.5 py-1.5 rounded-lg text-sm font-semibold transition-colors {{ app()->getLocale() === 'en' ? 'bg-slate-600 text-white' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white' }}">EN</a>
                </div>
                {{-- Menu dropdown with avatar --}}
                <div class="relative" id="driver-menu-wrap">
                    <button type="button" id="driver-menu-btn"
                        class="flex items-center gap-2 px-2 sm:px-3 py-2 rounded-full sm:rounded-xl border border-slate-300 dark:border-slate-600/50 hover:bg-slate-100 dark:hover:bg-slate-700/50 font-semibold text-sm transition-colors text-slate-600 dark:text-slate-300">
                ...
                <div id="driver-menu-dropdown" class="hidden absolute top-full end-0 mt-1 w-48 rounded-2xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600/50 shadow-lg py-1 z-50">
                    <a href="{{ route('driver.notifications.index') }}" class="flex items-center gap-3 px-4 py-3 hover:bg-slate-100 dark:hover:bg-slate-700/50 text-slate-700 dark:text-slate-300 font-medium">
                    ...
                    <a href="{{ url('/') }}" class="flex items-center gap-3 px-4 py-3 hover:bg-slate-100 dark:hover:bg-slate-700/50 text-slate-700 dark:text-slate-300 font-medium">
                        <i class="fa-solid fa-house w-5 text-center"></i>
                        {{ __('dashboard.main_page') }}
                    </a>
                    <form method="POST" action="{{ route('driver.logout') }}" class="block">
                        @csrf
                        <button type="submit" class="flex items-center gap-3 w-full px-4 py-3 hover:bg-slate-100 dark:hover:bg-slate-700/50 text-slate-700 dark:text-slate-300 font-medium text-start">
    ...
    <div class="flex-1 flex flex-col lg:flex-row min-w-0">
        ...
        <aside class="hidden lg:flex lg:flex-col w-64 shrink-0 bg-white dark:bg-slate-800/60 border-e border-slate-200 dark:border-slate-600/50 transition-colors duration-300">
            <nav class="flex-1 p-4 space-y-1 overflow-y-auto">
                <a href="{{ route('driver.dashboard') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl font-semibold transition-colors {{ request()->routeIs('driver.dashboard') ? 'bg-sky-500/20 text-sky-600 dark:text-sky-400' : 'hover:bg-slate-100 dark:hover:bg-slate-700/50 text-slate-600 dark:text-slate-300' }}">
                    <i class="fa-solid fa-house w-5 text-center"></i>
                    {{ __('driver.dashboard') }}
                </a>
                <a href="{{ route('driver.history') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl font-semibold transition-colors {{ request()->routeIs('driver.history') ? 'bg-sky-500/20 text-sky-600 dark:text-sky-400' : 'hover:bg-slate-100 dark:hover:bg-slate-700/50 text-slate-600 dark:text-slate-300' }}">
                    <i class="fa-solid fa-clock-rotate-left w-5 text-center"></i>
                    {{ __('driver.latest_requests') }}
                </a>
                <a href="{{ route('driver.notifications.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl font-semibold transition-colors {{ request()->routeIs('driver.notifications.*') ? 'bg-sky-500/20 text-sky-600 dark:text-sky-400' : 'hover:bg-slate-100 dark:hover:bg-slate-700/50 text-slate-600 dark:text-slate-300' }}">
                    <i class="fa-solid fa-bell w-5 text-center"></i>
                    {{ __('common.notifications') }}
                </a>
        ...
        @if (session('success'))
            <div id="toast-success" class="fixed bottom-24 end-4 lg:bottom-4 z-[100] max-w-sm rounded-2xl border border-emerald-500/40 bg-emerald-500/10 px-4 py-3 text-sm text-emerald-600 dark:text-emerald-400 font-medium shadow-lg backdrop-blur-sm flex items-center gap-2" role="alert">
                <i class="fa-solid fa-circle-check shrink-0"></i>
                <span>{{ session('success') }}</span>
            </div>
        @endif
        @if (session('error'))
            <div id="toast-error" class="fixed bottom-24 end-4 lg:bottom-4 z-[100] max-w-sm rounded-2xl border border-rose-500/40 bg-rose-500/10 px-4 py-3 text-sm text-rose-600 dark:text-rose-400 font-medium shadow-lg backdrop-blur-sm flex items-center gap-2" role="alert">
    ...
    <nav class="lg:hidden fixed bottom-0 inset-x-0 w-full z-40 bg-white/95 dark:bg-slate-800/95 border-t border-slate-200 dark:border-slate-600/50 backdrop-blur transition-colors duration-300" style="padding-bottom: max(env(safe-area-inset-bottom, 0px), 8px);">
        <div class="flex justify-around items-center h-[72px] min-h-[44px]">
            <a href="{{ route('driver.dashboard') }}" class="flex flex-col items-center justify-center flex-1 min-h-[44px] py-2 text-slate-500 dark:text-slate-400 active:scale-[0.98] {{ request()->routeIs('driver.dashboard') ? 'text-sky-600 dark:text-sky-400 font-bold' : 'hover:text-slate-900 dark:hover:text-white' }}">
                <i class="fa-solid fa-house text-xl mb-0.5"></i>
                <span class="text-xs">{{ __('driver.nav_home') }}</span>
            </a>
            <a href="{{ route('driver.history') }}" class="flex flex-col items-center justify-center flex-1 min-h-[44px] py-2 text-slate-500 dark:text-slate-400 active:scale-[0.98] {{ request()->routeIs('driver.history') ? 'text-sky-600 dark:text-sky-400 font-bold' : 'hover:text-slate-900 dark:hover:text-white' }}">
            ...
            <a href="{{ route('driver.fuel-refill.create') }}" class="flex flex-col items-center justify-center flex-1 min-h-[44px] py-2 text-slate-500 dark:text-slate-400 active:scale-[0.98] {{ request()->routeIs('driver.fuel-refill.*') ? 'text-amber-600 dark:text-amber-400 font-bold' : 'hover:text-slate-900 dark:hover:text-white' }}">
...
(function() {
    ...
    function showTrackingBar(vehicleId, displayName) {
        ...
        bar.innerHTML = '<div class="rounded-2xl shadow-lg border border-emerald-500/40 bg-emerald-500/10 flex items-center justify-between gap-3 px-4 py-3">' +
            ...
            '<span class="text-sm font-semibold text-emerald-600 dark:text-emerald-400 truncate">' + (displayName || '{{ __("tracking.tracking_active") }}') + '</span>' +
            '</div>' +

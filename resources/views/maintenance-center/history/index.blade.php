    ...
    <div class="dashboard-content max-w-7xl mx-auto space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="dash-card bg-sky-500/10 border-sky-500/30">
                <p class="text-slate-600 dark:text-servx-silver text-sm">{{ __('maintenance.total_jobs_filtered') ?? 'Total Jobs (Filtered)' }}</p>
            ...
            <div class="dash-card bg-emerald-500/10 border-emerald-500/30">
                <p class="text-slate-600 dark:text-servx-silver text-sm">{{ __('maintenance.total_revenue_filtered') ?? 'Total Revenue (Filtered)' }}</p>
        ...
        <div class="dash-card">
            ...
            <form method="GET" action="{{ route('maintenance-center.history.index') }}" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
                <div>
                    <label class="text-sm font-bold text-slate-700 dark:text-servx-silver-light block mb-1">{{ __('maintenance.company') ?? 'Company' }}</label>
                    <select name="company_id" class="w-full rounded-xl border border-slate-300 dark:border-slate-600/50 bg-white dark:bg-slate-800/60 px-4 py-2 text-slate-900 dark:text-servx-silver-light transition-colors duration-300">
                ...
                <div>
                    <label class="text-sm font-bold text-slate-700 dark:text-servx-silver-light block mb-1">{{ __('driver.vehicle') }}</label>
                    <select name="vehicle_id" class="w-full rounded-xl border border-slate-300 dark:border-slate-600/50 bg-white dark:bg-slate-800/60 px-4 py-2 text-slate-900 dark:text-servx-silver-light transition-colors duration-300">
                        <option value="">{{ __('common.all') }}</option>
                ...
                <div>
                    <label class="text-sm font-bold text-slate-700 dark:text-servx-silver-light block mb-1">{{ __('common.status') }}</label>
                    <select name="status" class="w-full rounded-xl border border-slate-300 dark:border-slate-600/50 bg-white dark:bg-slate-800/60 px-4 py-2 text-slate-900 dark:text-servx-silver-light transition-colors duration-300">
                        <option value="">{{ __('common.all') }}</option>
                ...
                <div class="flex items-end gap-2">
                    ...
                    <a href="{{ route('maintenance-center.history.index') }}" class="px-4 py-2 rounded-xl border border-slate-300 dark:border-slate-600/50 hover:bg-slate-100 dark:hover:bg-slate-700/50 font-bold text-slate-700 dark:text-servx-silver-light transition-colors duration-300">{{ __('common.reset') }}</a>
                </div>
            </form>
        </div>
        ...
        <div class="dash-card">
            @if($requests->isEmpty())
                <p class="text-slate-600 dark:text-servx-silver">{{ __('maintenance.no_history') ?? 'No maintenance history found.' }}</p>
            @else

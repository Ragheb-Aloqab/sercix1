    ...
    <div class="min-h-screen flex items-center justify-center px-4 py-10">
        <div class="w-full max-w-md">
            <div class="bg-servx-black-card rounded-xl border border-servx-red/30 p-6 sm:p-8">
                <h1 class="text-2xl font-bold text-white">{{ __('maintenance.verify_otp') ?? 'تأكيد رمز التحقق' }}</h1>
                <p class="mt-2 text-sm text-servx-silver">{{ __('maintenance.code_sent_to') ?? 'تم إرسال الرمز إلى' }}: <span class="font-bold">{{ $phone }}</span></p>
                ...
                <form method="POST" action="{{ route('maintenance-center.verify') }}" class="mt-6">
                    @csrf
                    <label class="block text-sm font-medium text-servx-silver-light">{{ __('maintenance.otp_code') ?? 'رمز التحقق (6 أرقام)' }}</label>
                    <input type="text" name="otp" inputmode="numeric" maxlength="6" required placeholder="123456" class="mt-2 w-full tracking-widest text-center text-2xl font-bold rounded-lg border border-servx-red/30 bg-servx-black-soft px-4 py-3 text-servx-silver-light">

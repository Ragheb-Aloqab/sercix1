...
@section('title', __('admin_dashboard.new_announcement') . ' | ' . ($siteName ?? 'Servx Motors'))
@section('page_title', __('admin_dashboard.new_announcement'))
...
<div class="dashboard-glass min-h-[calc(100vh-8rem)] mx-0 px-4 sm:px-6 py-6 sm:py-8 rounded-[28px] sm:rounded-[32px] overflow-hidden shadow-2xl">
    <div class="dashboard-content max-w-2xl mx-auto space-y-6">
        <div class="flex justify-between items-center">
            <h1 class="dash-page-title">{{ __('admin_dashboard.new_announcement') }}</h1>
        ...
        <div class="dash-card">
            <form method="POST" action="{{ route('admin.announcements.store') }}" class="space-y-4">
                ...
                <div>
                    ...
                    <input name="title" value="{{ old('title') }}" required class="mt-1 w-full px-4 py-2.5 rounded-xl bg-slate-800/50 border border-slate-600 text-white">
                    @error('title')<p class="mt-1 text-sm text-red-400">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="text-xs font-semibold text-slate-400">{{ __('admin_dashboard.body') }} *</label>
                    <textarea name="body" rows="5" required class="mt-1 w-full px-4 py-2.5 rounded-xl bg-slate-800/50 border border-slate-600 text-white">{{ old('body') }}</textarea>
                ...
                <div>
                    ...
                    <select name="target_type" id="target_type" class="mt-1 w-full px-4 py-2.5 rounded-xl bg-slate-800/50 border border-slate-600 text-white">
                        <option value="all" {{ old('target_type', 'all') === 'all' ? 'selected' : '' }}>{{ __('admin_dashboard.target_all') }}</option>
                        <option value="selected" {{ old('target_type') === 'selected' ? 'selected' : '' }}>{{ __('admin_dashboard.target_selected') }}</option>
                    </select>
                </div>
                <div id="companies_select" class="{{ old('target_type', 'all') === 'selected' ? '' : 'hidden' }}">
                    ...
                    <div class="mt-2 max-h-48 overflow-y-auto space-y-2 p-3 rounded-xl bg-slate-800/30">
                        @foreach($companies as $c)
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="checkbox" name="target_company_ids[]" value="{{ $c->id }}" {{ in_array($c->id, old('target_company_ids', [])) ? 'checked' : '' }}

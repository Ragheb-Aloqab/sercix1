<header
    class="sticky top-0 z-20 backdrop-blur border-b {{ auth('company')->check() ? 'bg-slate-900/70 border-slate-600/50' : 'bg-slate-50/70 dark:bg-slate-950/60 border-slate-200/70 dark:border-slate-800' }}">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 py-3 sm:py-4 flex items-center gap-2 sm:gap-3 min-w-0">
        {{-- Hamburger: mobile only --}}
        <button type="button"
                @click="sidebarOpen = true"
                class="lg:hidden flex items-center justify-center w-10 h-10 rounded-xl -ms-2 {{ auth('company')->check() ? 'text-slate-300 hover:bg-slate-700/50' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800' }}"

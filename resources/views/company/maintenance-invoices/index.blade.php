                ...
                <tbody>
                    @foreach($requests as $req)
                        <tr class="border-b border-slate-600/30 hover:bg-slate-800/30">
                            ...
                            <td class="py-6 pe-4">{{ $req->approvedCenter?->name ?? '-' }}</td>
                            <td class="py-6 pe-4">{{ $req->vehicle?->plate_number ?? '-' }}</td>
                            <td class="py-6 pe-4">{{ $req->approved_quote_amount ? number_format($req->approved_quote_amount, 2) . ' ' . (__('company.sar') ?? 'ر.س') : '-' }}</td>
                            <td class="py-6 pe-4">{{ $req->final_invoice_amount ? number_format($req->final_invoice_amount, 2) . ' ' . (__('company.sar') ?? 'ر.س') : '-' }}</td>
                            <td class="py-6 pe-4">{{ $req->final_invoice_uploaded_at?->format('Y-m-d H:i') ?? '-' }}</td>
                            <td class="py-6 pe-4">
                                @php $paid = $req->status === 'closed' && $req->invoice_approved_at; @endphp
                                <span class="px-2 py-1 rounded-xl text-xs font-bold {{ $paid ? 'bg-emerald-500/30 text-emerald-300 border border-emerald-400/50' : 'bg-amber-500/20 text-amber-400 border border-amber-400/50' }}">
                                    {{ $paid ? __('fleet.paid') : __('fleet.unpaid') }}
                                </span>
                            </td>
                            <td class="py-6 pe-4">
                                <div class="flex gap-2">
                                    <a href="{{ route('company.maintenance-invoices.view', $req) }}" target="_blank" class="px-3 py-2 rounded-xl bg-sky-600 hover:bg-sky-500 text-white text-sm font-semibold">
                                        <i class="fa-solid fa-eye me-1"></i> {{ __('common.view') }}
                                    </a>
                                    <a href="{{ route('company.maintenance-invoices.download', $req) }}" class="px-3 py-2 rounded-xl border border-slate-600/50 hover:bg-slate-700/50 text-servx-silver-light text-sm font-semibold">

...
<head>
    ...
    @include('components.seo-meta', [
        'title' => (app()->getLocale() === 'ar' ? 'تأكيد OTP' : 'Verify OTP') . ' — ' . ($siteName ?? 'Servx Motors'),
...
<div class="min-h-screen flex flex-col items-center justify-center px-4 py-10 relative">
    ...
    <div class="w-full max-w-md">
        ...
        <div class="bg-white dark:bg-servx-black-card rounded-xl border border-slate-200 dark:border-servx-red/30 shadow-lg dark:shadow-servx-card p-6 sm:p-8 transition-colors duration-300">
            <h1 class="text-2xl font-bold text-slate-900 dark:text-white">{{ app()->getLocale() === 'ar' ? 'تأكيد رمز التحقق' : 'Verify code' }}</h1>
            <p class="mt-2 text-sm text-slate-600 dark:text-servx-silver">
                {{ app()->getLocale() === 'ar' ? 'تم إرسال رمز إلى:' : 'Code sent to:' }} <span class="font-bold text-slate-900 dark:text-servx-silver-light">{{ $phone }}</span>
                @if(app()->environment('local'))
                    <br><span class="text-xs text-slate-500 dark:text-servx-silver">({{ app()->getLocale() === 'ar' ? 'تجريبي: الرمز موجود في ملف log' : 'Demo: check log file' }})</span>
                @endif
            </p>

            @if (session('success'))
                <div class="mt-4 rounded-lg border border-emerald-400/50 dark:border-servx-red/30 bg-emerald-50 dark:bg-servx-red/10 px-3 py-2 text-sm text-emerald-800 dark:text-servx-silver-light">
                    {{ session('success') }}
                </div>
            @endif

            @if ($errors->any())
                <div class="mt-4 rounded-lg border border-rose-400/50 dark:border-servx-red/50 bg-rose-50 dark:bg-servx-red/10 px-3 py-2 text-sm text-rose-800 dark:text-servx-silver-light">
            ...
            <form method="POST" action="{{ route('company.verify_otp') }}" class="mt-6 space-y-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-600 dark:text-servx-silver-light">{{ app()->getLocale() === 'ar' ? 'رمز التحقق (6 أرقام)' : 'Verification code (6 digits)' }}</label>
                    ...
                    <p class="mt-2 text-xs text-slate-500 dark:text-servx-silver">{{ app()->getLocale() === 'ar' ? '* أدخل الرمز قبل انتهاء الصلاحية.' : '* Enter the code before it expires.' }}</p>
                </div>

                <button type="submit"
                    class="w-full rounded-lg px-4 py-3 min-h-[44px] text-sm font-bold text-white transition-all duration-200 hover:scale-[1.02] active:scale-[0.99] {{ ($wlBranding ?? false) ? 'auth-btn-primary' : 'bg-servx-red hover:bg-servx-red-hover' }}">
                    {{ app()->getLocale() === 'ar' ? 'تحقق ودخول' : 'Verify & sign in' }}
                </button>

                <div class="mt-4 space-y-2">
                    <p id="otp-timer" class="text-sm text-slate-600 dark:text-servx-silver">
                        {{ app()->getLocale() === 'ar' ? 'إعادة الإرسال خلال:' : 'Resend in:' }} <span id="otp-timer-value" class="font-mono font-bold text-slate-900 dark:text-servx-silver-light">02:00</span>
                    </p>
                    <div id="otp-resend-wrap" class="flex items-center justify-between text-sm" style="display:none">
                        <a href="{{ route('sign-in.index') }}" class="font-medium text-slate-600 dark:text-servx-silver hover:text-sky-600 dark:hover:text-servx-red transition-colors duration-300">
                        ...
                        @if($isRegistration ?? false)
                            <form method="POST" action="{{ route('company.resend_register_otp') }}" class="inline">
                                @csrf
                                <button type="submit" id="otp-resend-btn" class="font-medium text-sky-600 dark:text-servx-red hover:text-sky-700 dark:hover:text-servx-red-hover transition-colors duration-300">

...
<div class="flex flex-wrap items-center justify-between gap-3 mb-6">
    ...
    <div class="flex flex-wrap gap-2">
        <x-export-dropdown
            :pdfUrl="route('company.reports.tax.pdf', request()->query())"
            :excelUrl="route('company.reports.tax.excel', request()->query())"
        />
    </div>
</div>

{{-- Filters --}}
<form method="GET" action="{{ route('company.reports.tax') }}" class="rounded-2xl bg-white dark:bg-slate-800/40 border border-slate-200 dark:border-slate-500/30 p-4 sm:p-5 backdrop-blur-sm mb-6 transition-colors duration-300">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
        ...
        <div>
            ...
            <select name="vehicle_id" class="w-full rounded-xl border border-slate-300 dark:border-slate-600/50 bg-white dark:bg-slate-800/60 text-slate-900 dark:text-white px-4 py-2 transition-colors duration-300">
        ...
        <div class="flex items-end">
            <button type="submit" class="w-full px-4 py-2 rounded-xl bg-sky-600 hover:bg-sky-500 text-white font-bold transition-colors duration-300">
